...
出发确认框可选择酒馆，并预览其收费与风声。已接通各自货架、预约与特殊路线、存档身份及新局线索隔离。`scene_rules_test.gd` 的 176 项检查通过。目前四种选择共用建筑，独特美术仍待完成，不能把它当作四套完整场景。

## 搜索点

每间牌桌房间后方左角的柜子可对准按 E 查看搜索选择。查看免费，确认消耗一点行动力，收益和代价提前显示；每处每局仅一次，读档保留结果。执行 `search_events_test.gd -- --test` 验证，90 项通过。当前事件为固定设计，柜子与信件仍是待细化模型。
