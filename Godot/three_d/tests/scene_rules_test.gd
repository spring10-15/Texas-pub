...
func run() -> void:
	...
	for i in range(world.scene_choice.item_count):
		...
		verify(world.get_node("Tavern/KitchenExit").title == "后厨楼梯 · 线索未明" and world.get_node("Tavern/FixedExit").title == "库房货梯 · 线索未明", "Unknown physical routes use structural clues")
		world.show_run_panel("route:fixed")
		verify(world.run_heading.text == "库房货梯 · 路线未确认" and world.run_body.text.contains("取得线索后") and not world.run_body.text.contains("最终到账") and world.run_confirm.disabled, "Unknown route does not disclose settlement")
