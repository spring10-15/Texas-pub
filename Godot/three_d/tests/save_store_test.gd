...
func _initialize() -> void:
	...
	record("read_version", version_result.status == "unsupported_version" and version_result.version == 99)
	file = FileAccess.open(path, FileAccess.WRITE)
	file.store_buffer(PackedByteArray([1, 2, 3]))
	file.close()
	var short_status: String = Store.read_checkpoint(path).status
	...
	file.close()
	record("read_truncated", short_status == "invalid" and Store.read_checkpoint(path).status == "invalid")
