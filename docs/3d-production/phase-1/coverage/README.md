...
> 阅读口径（2026-09-25）：下文按实现批次保留历史快照，226、232、239、247、252 等数字各对应当时状态。最新登记数为 359，当前登记证据为 359/359；全局目录仍未完成，整体覆盖率为空。胜者与边池分配已有下文 payout 六种结果的独立金额测试；这不代表穷尽所有牌型和下注路径。

2026-09-25 新增表内检查点 RNG 延续结果 `persistence_table.rng_replay`。`table_checkpoint_test.gd` 在两张桌、10 个种子下逐个动作边界写入、恢复并同步推进；牌局状态、牌堆、后续手牌与 RNG 状态完全一致，共 660 个动作边界、2,031 项检查通过。同日新增旧计划版本迁移结果 `persistence_run.legacy_variant_plan_restored`，通过版本 2/3 内存旧计划验证历史对手阵容与线性房间图，82 项检查通过；不代表验证过历史磁盘旧档。新增后完整回归 59/59、覆盖汇总 `verified=359 catalogued=359`，覆盖汇总器单测 3/3 通过。`pending_families` 仍包含世界交互与存档各阶段，整体覆盖率继续为 unavailable，不能据此宣称 Phase 1 通过。报告：`output/3d/regression/20260925-025539/report.json`。
