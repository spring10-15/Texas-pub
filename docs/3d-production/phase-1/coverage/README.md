...
2026-09-25 全局审计增量：新增 `start.partial_bankroll`、`entry.heat_cap`、`settlement.heat_relief`、`poker.player_raise_pattern` 与 `run_variant.room_layout_selected`。部分本金用 vault=120 验证金库归零、现金/本金均为 120、财富守恒、Run 激活及 revision 增加；屋顶会所风声 5 入座后验证封顶为 6；盈利结算余烬桌验证 heat 按 `winHeatRelief` 从 2 降到 1，并检查现金回收、桌面清理、完成标记和 revision；玩家 raise/all-in 会更新玩家行为画像，对手加注不会误记为玩家行为；四家酒馆都验证房间图种子生成、对不同布局产生不同解锁要求，并在 Run 快照恢复后保持一致。搜索点与货架的真实交互锚点测试复用既有 `world.services_open` ID，不扩大分母。全量回归 60/60，报告 `output/3d/regression/20260925-121929/report.json`；覆盖汇总与汇总器单测通过，当前登记证据 387/387。A8 当前树仍有 26 条可达候选与 18 条弱证据项待审；全局分母未知，不能据此宣称整体覆盖率。
