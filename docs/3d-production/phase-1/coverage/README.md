...
> 阅读口径（2026-09-25）：下文按实现批次保留历史快照，226、232、239、247、252 等数字各对应当时状态。最新登记数为 394，全部 394 项均有当前证据；扑克、世界、存档三个已审分组虽已清空待办，但全局源码入口与可达结果尚未完成独立分母审计，因此整体覆盖率仍为空。胜者与边池分配已有下文 payout 六种结果的独立金额测试；这不代表穷尽所有牌型和下注路径。

2026-09-25 全局审计增量：新增 `start.partial_bankroll`、`entry.heat_cap`、`settlement.heat_relief`、`poker.player_raise_pattern`、`run_variant.room_layout_selected`、`poker_blind.short_stack_posts` 与 `poker_progress.seeded_deal`。部分本金用 vault=120 验证金库归零、现金/本金均为 120、财富守恒、Run 激活及 revision 增加；屋顶会所风声 5 入座后验证封顶为 6；盈利结算余烬桌验证 heat 按 `winHeatRelief` 从 2 降到 1，并检查现金回收、桌面清理、完成标记和 revision；玩家 raise/all-in 会更新玩家行为画像，对手加注不会误记为玩家行为；四家酒馆都验证房间图种子生成、对不同布局产生不同解锁要求，并在 Run 快照恢复后保持一致。搜索点与货架的真实交互锚点测试复用既有 `world.services_open` ID，不扩大分母。房间图测试另验证 linear/fork 下锁门入座均被拒绝且完整 Run checkpoint 不变，复用 `entry.locked`。四桌的短筹码小盲与大盲均验证实际缴纳额受栈封顶、仍发底牌、不标为弃牌且总筹码守恒；同种子起手发牌逐座对照独立重算的精确牌序、剩余牌堆与 RNG 状态。开注测试补充断言 `raiseUsed` 未被错误置位，归属复用 `poker_action.open`。输入链路测试用真实 Esc KeyEvent 验证暂停信号和 World 暂停状态；E 键输入映射及 Player→World 交互信号下游通过台灯后置验证，但 headless 无法进入鼠标捕获模式，仍留一条弱证据。另补焦点重复刷新测试，确认相同目标不会重复发送 UI 信号；此分支不改权威状态，A8 将其归类为非状态转移。回归脚本默认超时由 90 秒提升至 180 秒后，全量回归 61/61，报告 `output/3d/regression/20260925-132741/report.json`；覆盖汇总与汇总器单测通过，当前登记证据 389/389。A8 当前树仍有 18 条可达候选与 19 条弱证据项待审；全局分母未知，不能据此宣称整体覆盖率。

自动存档调度补测：在隔离 `user://` 文件下直接执行 `World._process(0.6)`，断言超过 0.5 秒间隔后时钟归零、磁盘存档与完整内存 checkpoint 相等、提示更新且临时文件删除；新增 `world.autosave`。当前覆盖汇总 390/390，全量回归 61/61，报告 `output/3d/regression/20260925-134024/report.json`；A8 overlay 仍标出 17 条可达候选与 19 条弱证据项，整体分母依然未知。

窗口失焦处理补测：新增独立的非 `--test` Godot 套件，并由回归 runner 为该套件提供隔离 HOME。套件投递 `NOTIFICATION_WM_WINDOW_FOCUS_OUT`，确认服务面板关闭、游戏暂停、玩家控制关闭，且失焦前完整 checkpoint 已保存到隔离路径；新增 `world.window_focus_out`。此验证覆盖 Godot 通知处理函数，不替代人工 Alt-Tab 系统集成检查。

2026-09-25 收尾复验：玩家视角、碰撞移动、Player→World 交互链与关窗写盘加入后，完整回归 64/64，通过报告 `output/3d/regression/20260925-143046/report.json`；覆盖汇总 394/394，Python 单测 6/6，A4 世界审计与 A5 存档映射自检均通过。A8 当前树重新归因仍有 12 条可达候选和 18 条弱证据项；全局分母和整体覆盖率仍未知。鼠标输入测试使用桌面渲染器和注入事件，窗口通知也以处理函数注入验证，没有声称完成原生系统事件手测。
