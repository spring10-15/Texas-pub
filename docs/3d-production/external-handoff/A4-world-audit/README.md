...
> **主线当前状态复核（2026-09-25，当前工作树）**：A4 CSV 72 行中 70 行有正式测试后继证据，探针证据已清零；其余 2 行均判为正常玩家路径不可达（未注册道具 ID、世界与 Run 桌实例失同步后的结算失败）。后者的判定依据是正常入桌与存档恢复均维护同一桌实例，离座用当前 revision 结算且先要求桌已结束。70 行映射到现有目录 ID，没有新建重复分母。新增的 `world.leave_forced_pressure_exit` 验证真实入座后离座触发强制撤离；静态遮挡清焦、入座时锚点拒绝、暂停时确认和服务动作拒绝也有正式后继断言。A4 verifier 通过；世界专项 72/72、全回归 59/59。`pending_families.world` 仍需完整枚举。
