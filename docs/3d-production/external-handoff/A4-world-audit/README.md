...
> **主线当前状态复核（2026-09-25，当前工作树）**：A4 CSV 72 行中 70 行有正式测试后继证据，探针证据已清零；另有 1 行不可达、1 行可达性未证实。70 行映射到现有目录 ID，没有新建重复分母。新增的 `world.leave_forced_pressure_exit` 验证真实入座后离座触发强制撤离；静态遮挡清焦、入座时锚点拒绝、暂停时确认和服务动作拒绝现也有正式后继断言。A4 verifier 通过；世界专项 72/72、全回归 59/59。`pending_families.world` 仍需完整枚举。
