...
2. 以当前 383 项目录为新快照基线，复核 `branch-inventory.csv` 中的每个目录 ID 至少有一条有效归因；将 `start.partial_bankroll` 对应的 `run.gd:61-62` 行改记为已登记，并附当前测试和回归报告。
...
- `current-tree-unmapped-review.csv`
...
- 383 个当前目录 ID 全部且仅按有效行映射；没有旧 `382/382` 被误称为当前结论。
- 所有“未映射可达”行均有 `player_reachable=yes` 或逐条解释为何属游戏调度可达；不可达/未知项不计入可达缺口总数。
- `start.partial_bankroll` 有当前强证据归因。
