...
逐条复核 `current-tree-player-path-gaps.csv` 中的 31 行候选，判断它们是否确实是玩家/正常游戏调度可达、会改变权威状态、且没有被现有目录 ID 覆盖的独立结果。交付证据和分类，供主 Agent 决定是否增补目录与测试。

## 基线与边界

- 当前对账：`current-tree-reconciliation.md`，基线 HEAD `7202b01`，目录 384 项；如开始时 HEAD/目录哈希已变化，先记录新值，不要改写对账基线。
- 输入：`current-tree-player-path-gaps.csv`。必须核验文件中的全部行；实际行数以 CSV 为准。
...
1. 读取 31 行候选及对应源码完整函数，记录当前源文件 SHA-256 和逐行位置。
...
4. 与当前 384 个目录 ID、现有测试和覆盖报告比对，确认是否已有 ID 覆盖同一结果。明确区分“缺 ID”与“已有 ID 但测试证据不足”。
