...
> 状态：已完成。审计开始时核验输入 30 行；其中 1 行映射到 `poker.player_raise_pattern`；搜索与货架入口已补强既有 `world.services_open` 的测试证据，不新增目录 ID。当前树对账剩 27 条可达无 ID 候选、18 条弱证据项。历史 30 行逐条结果保留在 `current-tree-gap-triage.csv` / `.md`，当前基线见 `current-tree-reconciliation.md`。
