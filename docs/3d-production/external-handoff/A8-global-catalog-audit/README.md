...
**当前树复核**：后续又登记了 `entry.heat_cap`、`settlement.heat_relief`、`poker.player_raise_pattern` 与 `run_variant.room_layout_selected`，目录现为 387 个 ID；搜索与货架入口证据归入既有 `world.services_open`，房间图锁门拒绝证据归入既有 `entry.locked`，均未新增 ID。请使用 [当前树对账记录](current-tree-reconciliation.md) 与 `current-tree-branch-inventory.csv`；当前 387/387 个 ID 均可归因，另有 24 条可达无 ID 候选和 18 条弱证据项尚待逐条审查。下文的原始报告、CSV 与 `build_a8.py --check` 仍用于复现 382 项冻结锚点，不代表当前分母闭合。
