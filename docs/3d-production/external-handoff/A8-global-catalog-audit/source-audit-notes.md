...
后续登记了 `start.partial_bankroll`、`entry.heat_cap`、`settlement.heat_relief`、`poker.player_raise_pattern` 与 `run_variant.room_layout_selected`；当前目录为 387 个 ID。部分本金、屋顶会所风声封顶、盈利余烬桌降风声、玩家加注画像与房间图选择都有具体后继断言；搜索/货架入口测试复用 `world.services_open` ID。此段证据状态随最新回归报告刷新；全局分母仍未冻结。
