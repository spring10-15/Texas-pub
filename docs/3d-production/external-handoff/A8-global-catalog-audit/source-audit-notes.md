...
后续登记了 `start.partial_bankroll`、`entry.heat_cap`、`settlement.heat_relief` 与 `poker.player_raise_pattern`；当前目录为 386 个 ID。部分本金、屋顶会所风声封顶、盈利余烬桌降风声与玩家加注画像都有具体后继断言；搜索/货架入口测试复用 `world.services_open` ID。此段证据状态随最新回归报告刷新；全局分母仍未冻结。
