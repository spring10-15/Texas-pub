...
> **当前源码与目录校订（2026-09-25）**：近期 `world.gd` 在存档函数前增加试玩轨迹记录，导致本文件 19 条 `source_line` 偏移 3 行；已按原内容锚点刷新，行为未因此变化。A5 强后继证据现映射 38 个目录 ID，包括世界/运行/牌桌快照隔离、原子写入失败、畸形快照拒绝、损坏/未来版本保留、锁定房间拒绝、缺字段恢复、旧 search_results 事件字段回填、表内 RNG 重放、版本 2/3 计划回填、缺档与损坏档恢复、试玩模式禁用存档，以及活动桌未入座和桌型/房间不匹配拒绝。相同语义的多个输入或负对照复用 ID，不扩大分母。版本迁移检查使用运行时生成的 version 1 旧形态磁盘夹具，不声称读取真实历史用户存档。恢复子图 18/18（59 项检查）；快照 capture 子图 3/3；全量回归 59/59，目录证据 369/369。其他存档 family 仍待逐项审查，整体覆盖率不可用。
...
### capture（目录里 3 条）
1. `world.gd::checkpoint_state()` @944 — accepted，返回 `{run,room,player,look,seated,return,caseOpen,props}`；`persistence_capture.world_snapshot_isolated` 直接验证 Run/props 嵌套拷贝与 live 状态隔离。
2. `run_checkpoint.gd::capture()` @6 — accepted，31 个 FIELDS 深拷贝 + `table` 子快照；`persistence_capture.run_snapshot_isolated`。
3. `table_checkpoint.gd::capture()` @5 — accepted，`{state,revision,rngValue}`，`rngValue` 是重放关键字段；`persistence_capture.table_snapshot_isolated`。
...
15. payload 解码为非 Dictionary → `invalid`；`persistence_io.read_non_dictionary`；拒绝后原文件字节不变。
...
- `README.md`：本文件。
- `outcomes.csv`：原始审计初版 49 行；当前 50 行，七组齐全（capture 3 / write 6 / read 8 / restore 7 / invalid_data 17 / legacy_migration 6 / rng_replay 3），38 个目录 ID 全映射，1 行 `unverified`。
- `repro/`（仅新增诊断，均只读/隔离）：
...
收尾自检 `repro/verify_outcomes.py` 当前结果：`rows=50`、`catalog_ids_mapped=37/37`、所有 `evidence_path` 存在、`source_line` 按内容锚点命中、七组齐全、CSV 可被 `csv` 解析且列数一致。
...
2026-09-25 当前检查补充：`world_restore_atomic_test.gd` 对“活动牌桌但 seated=false”和“活动桌与已解锁房间桌型不一致”新增正式后继断言；两种输入均拒绝且恢复前完整世界状态保持不变。`persistence_capture_coverage_test.gd` 新增 `persistence_capture.world_snapshot_isolated`，验证世界快照内嵌的 Run 与 props 深拷贝：改返回快照不改变 live 状态，后续 live 变更也不反向修改既有快照，并把 `world.gd` 纳入该报告的新鲜度哈希。A5 当前核验为 50 行、38/38 ID 映射；capture 子图 3/3、恢复子图 18/18（59 项检查），全回归 59/59，目录证据 369/369。报告见 `output/3d/regression/20260925-044032/report.json`。

2026-09-25 补充：`save_store_test.gd` 使用正确版本与摘要封装一个 Array payload，确认 `read_checkpoint()` 返回 `invalid` 且拒绝后原文件字节保持不变；新增目录 ID `persistence_io.read_non_dictionary`。专项 10/10、A5 映射 37/37、全量回归 59/59、全目录证据 368/368。全局分母仍未封板。
