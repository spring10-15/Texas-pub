...
> **当前源码与目录校订（2026-09-25，基线 `833f76c`）**：近期 `world.gd` 在存档函数前增加试玩轨迹记录，导致本文件 19 条 `source_line` 偏移 3 行；已按原内容锚点刷新，行为未因此变化。A5 强后继证据已复用或登记 23 个现有目录 ID：包括运行/牌桌 capture 隔离、畸形快照拒绝、锁定房间拒绝、缺字段恢复、表内 RNG 重放，以及版本 2/3 计划回填；同语义的多个畸形输入复用同一 ID，不扩大分母。版本迁移检查使用内存旧格式样本，不声称读取真实历史磁盘存档。全量回归 59/59，目录证据 359/359；其他存档 family 仍待逐项审查。
